"""
Fixed CLOUDS simulation / fitting script.

Main fixes relative to the original version:
  1. Uses raw time for dynamics and scaled time only for the age/covariate trend.
...
# Blind PyTorch to CUDA before importing torch.
...
# Keep CPU kernels from thread-thrashing. Worker processes also set this.
torch.set_num_threads(8)

# ---------------------------------------------------------------------
# 0. Logging
# ---------------------------------------------------------------------
LOG_FILENAME = "clouds_anchor_simulation3.log"
logging.basicConfig(
    level=logging.INFO,
...
class CLOUDS(nn.Module):
    ...
    def _extract_times(self, subj: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        t_dyn is the time scale used for dt in the OU transition.
        t_trend is the centered/scaled time used in the covariate trend.
        """
        if "t_dyn" in subj:
            t_dyn = subj["t_dyn"]
        elif "t" in subj:
            # Backward-compatible fallback for old data dictionaries.
            t_dyn = subj["t"]
        else:
            raise KeyError("Subject dictionary must contain 't_dyn' or legacy key 't'.")

        if "t_trend" in subj:
            t_trend = subj["t_trend"]
        elif "t" in subj:
            t_trend = subj["t"]
        else:
            t_trend = t_dyn

        return t_dyn, t_trend
    ...
    def get_subject_matrices(
        ...
        u_t = u[1:]
        trend_t = t_trend[1:].unsqueeze(1)
        mu = (u_t @ self.Phi_int.T + self.alpha_bias) * trend_t
        b_shift = torch.bmm(
            eye.unsqueeze(0).expand(n_trans, self.K, self.K) - A_trans,
            mu.unsqueeze(-1),
        ).squeeze(-1)
    ...
    def kalman_smoother(
        ...
        # Initial latent prior. Keeping identity preserves the spirit of the original
        # simulation, where the baseline factor is near zero rather than stationary.
    ...
    def _loss_normalizer(self, subjects_data: Sequence[Dict[str, torch.Tensor]]) -> float:
        ...
        for s in subjects_data:
            ...
            latent_cells += max(int(s["x"].shape[0]) - 1, 0) * self.K
...
def simulate_ad_cohort_stress(
    ...
    *,
    ...
    """Synthetic cohort generator with separate raw dynamics time and scaled trend time."""
    if theta_mode not in {"exact", "diagonal"}:
        raise ValueError("theta_mode must be 'exact' or 'diagonal'.")
    ...
    if theta_mode == "diagonal":
        ...
        Gamma_true = cholesky_right_solve_spd(S_true + A_true, Omega_true, jitter=1e-6)
    ...
    for _ in range(N):
        ...
        t_dyn = torch.cat([age_baseline.view(1), age_baseline + torch.cumsum(dt_years, dim=0)])
        t_trend = (t_dyn - 70.0) / 10.0

        u = torch.randn(J_i, C_dim)
        F_true = torch.zeros(J_i, K)
        F_true[0] = 0.1 * torch.randn(K)

        for j in range(1, J_i):
            delta_t = t_dyn[j] - t_dyn[j - 1]
            if theta_mode == "diagonal":
            ...
            mu_j = (Phi_true @ u[j] + alpha_true) * t_trend[j]
            L_Q = safe_cholesky(Q_true, jitter=1e-6)
            process_noise = L_Q @ torch.randn(K)
            F_true[j] = A_ij @ F_true[j - 1] + (eye - A_ij) @ mu_j + process_noise
...
def run_stress_test_multiprocessing(n_runs: int = 2) -> None:
    ...
    logging.info("Active Python processes: %s; PyTorch threads per worker: 8", max_workers)
    logging.info("-" * 132)
