...
model {
    ...
    for (i in 1:Nsub) {
        ...
        vector[R] mu_start = gamma_latent * Z[start_idx]';
        ...
        for (j in 2:repme[i]) {
            ...
            vector[R] target_mean = gamma_latent * Z[k]';
            vector[R] cond_mean = target_mean + Phi * (xi[k-1] - gamma_latent * Z[k-1]');
            
            xi[k] ~ multi_normal(cond_mean, add_diag(Q, 1e-9));
        }
    }
