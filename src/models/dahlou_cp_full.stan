...
parameters {
    ...
    // --- CP: Directly declare Latent States ---
    matrix[R, N] xi;
}
...
model {
    ...
    // --- CP: Latent Trajectories Likelihood ---
    {
        matrix[R, R] L_Omega = cholesky_decompose(Omega);
        for (i in 1:Nsub) {
            int start_idx = cumu[i] - repme[i] + 1;
            
            // --- UPDATED: Time 1 shifted by alpha_0 and scaled by absolute time ---
            vector[R] mu_start = (alpha_0 + alpha_dyn * X_dyn[start_idx, ]') * t_abs[start_idx];
            xi[:, start_idx] ~ multi_normal_cholesky(mu_start, L_Omega);
            ...
            for (j in 2:repme[i]) {
                ...
                // --- UPDATED: Times k and k-1 shifted by alpha_0 and scaled by absolute time ---
                ...
                xi[:, k] ~ multi_normal_cholesky(mu_k + Phi * (xi[:, k-1] - mu_prev), L_Q);
            }
        }
    }
