...
# 0. Logging
# ---------------------------------------------------------------------
LOG_FILENAME = "clouds_simulation_exact_improved.log"
logging.basicConfig(
    level=logging.INFO,
...
def one_standard_error_choice(results: Sequence[Dict[str, object]]) -> Dict[str, object]:
    """Choose the smallest K within one SE of the best mean validation log likelihood."""
    ...
    threshold = float(best["val_ll_per_cell_mean"]) - float(best["val_ll_per_cell_se"])
    eligible = [r for r in successful if float(r["val_ll_per_cell_mean"]) >= threshold]
...
def run_k_selection_experiment(
    *,
    ...
    chosen = one_standard_error_choice(rows)
    ...
    header = (
        ...
        f"{'Anchor stability':<16} | {'Avg time(s)':>11} | Example anchor groups"
    )
    logging.info(header)
    logging.info("-" * 160)
    for r in rows:
        ...
        stab = float(r["mean_anchor_stability"])
        stab_str = "nan" if not math.isfinite(stab) else f"{stab:.3f}"
        logging.info(
            f"{int(r['K']):>4} | {str(r['mode']).capitalize():<9} | {int(r['n_success']):>4} | "
            f"{val_str:<28} | {stab_str:<16} | {float(r['mean_time']):>11.1f} | {r.get('example_anchors')}"
        )
    logging.info("-" * 160)
    logging.info(
        "Selected by one-SE rule: K=%s, mode=%s. Best mean candidate was K=%s, mode=%s. Threshold=%.4f",
        int(chosen["K"]),
        str(chosen["mode"]),
