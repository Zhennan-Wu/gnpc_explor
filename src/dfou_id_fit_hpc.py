...
# ---------------------------------------------------------
# 1. GPU-Accelerated Universal DFOULS Model (Baseline)
# ---------------------------------------------------------
...
def run_single_task(task_id, n_runs, out_dir):
    ...
    # Execute the baseline fit_em method
    smoothed_stats, final_loss, loss_history = model.fit_em(
        subjects_data, num_em_epochs=100, m_step_iters=20, lr=0.01, verbose=True
    )
...
def merge_results(out_dir):
    ...
    # Utilizing id_fit filenames as specified in your base script
    df_summary.to_csv("metrics_summary_id_fit.csv", index=False)
    torch.save(parameter_archive, "parameter_archive_id_fit.pt")
    
    print(f"[+] Merged {len(file_paths)} tasks.")
    print("[+] Saved 'metrics_summary_id_fit.csv' and 'parameter_archive_id_fit.pt'")
