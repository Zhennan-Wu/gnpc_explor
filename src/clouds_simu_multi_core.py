...
def run_single_simulation(task_id, scenario, mode, run_idx, seed):
    """
    This function represents a single, isolated process. 
    It runs one simulation from start to finish silently, 
    and returns the metrics to the main thread.
    """
    # Force this specific worker to only use 8 threads to prevent thrashing
    torch.set_num_threads(8)
...
if __name__ == "__main__":
    if run_smoke_test():
        # Call the new Multiprocessing wrapper instead!
