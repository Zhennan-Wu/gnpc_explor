// Model 2a: Longitudinal IRT with SDE Latent Process & Covariates (Joint Latent Sampler)
// Updated for K=12 items (5 Binary, 7 Ordinal) and R=4 Latent Dimensions
// Note: This specific model uses the non-centered sampler and omits the c_latent parameter.
...
transformed parameters {
    ...
    // Continuous Lyapunov target mapping
    ...
    {
        // Cholesky factor of the stationary covariance
        matrix[R, R] L_Omega = cholesky_decompose(Omega);
        
        for (i in 1:Nsub) {
            int start_idx = cumu[i] - repme[i] + 1;
            
            // Time = 1 (No c_latent in this model variation)
