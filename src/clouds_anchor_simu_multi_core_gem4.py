...
# ---------------------------------------------------------
LOG_FILENAME = "clouds_anchor_simulation_gem4.log"
logging.basicConfig(
    level=logging.INFO,
...
# ---------------------------------------------------------
# 4. K-Scaling Evaluation (128-Core Multiprocessing)
# ---------------------------------------------------------

def run_single_simulation(task_id, DGP_mode, FIT_mode, scenario, run_idx, seed):
    torch.set_num_threads(8)
    logging.info(f"[Worker {task_id}] Scenario: K={scenario['K']} | {DGP_mode.upper()} -> {FIT_mode.upper()}...")
...
def run_k_scaling_stress_test(n_runs=10):
    # N is scaled up for large K to prevent catastrophic Exact mode overfitting
    scenarios = [
        {"name": "1. Base Latent Space (K=10)",     "N": 100, "D": 2000, "K": 10,  "C": 2, "miss": 0.0, "noise": 1.0},
        {"name": "2. High Latent Space (K=25)",     "N": 200, "D": 2000, "K": 25,  "C": 2, "miss": 0.0, "noise": 1.0},
        {"name": "3. Extreme Latent Space (K=50)",  "N": 300, "D": 2000, "K": 50,  "C": 2, "miss": 0.0, "noise": 1.0},
        {"name": "4. Massive Latent Space (K=100)", "N": 500, "D": 2000, "K": 100, "C": 2, "miss": 0.0, "noise": 1.0},
    ]
    ...
    logging.info("\n" + "=" * 120)
    logging.info(f"{'Scenario':<32} | {'DGP':<10} | {'FIT':<10} | {'Λ Corr':<15} | {'F Corr':<15} | {'Γ Corr':<15} | {'Avg Time (s)'}")
    logging.info("-" * 120)
    ...
    for s in scenarios:
        for dgp in ["exact", "diagonal"]:
            for fit in ["exact", "diagonal"]:
                ...
                logging.info(f"{s['name']:<32} | {dgp.capitalize():<10} | {fit.capitalize():<10} | {l_str:<15} | {f_str:<15} | {g_str:<15} | {np.mean([r['time'] for r in runs]):>8.1f}")
    logging.info("=" * 120)

if __name__ == "__main__":
    import multiprocessing as mp
    mp.set_start_method('spawn', force=True)
    run_k_scaling_stress_test(n_runs=10)