...
# ---------------------------------------------------------
LOG_FILENAME = "clouds_anchor_simulation_gem3.log"
logging.basicConfig(
    level=logging.INFO,
...
class CLOUDS(nn.Module):
    ...
    def get_subject_matrices(self, Gamma, Omega, u, t_dyn, t_trend):
        ...
        # --- SDE CORRECTED SHIFT LOGIC ---
        # 1. Compute the drift slopes (c) for t_j and t_{j-1}
        c_j = u[1:] @ self.Phi_int.T + self.alpha_bias
        c_jm1 = u[:-1] @ self.Phi_int.T + self.alpha_bias
        
        # 2. Evaluate the mean trajectory at exact timepoints
        mu_j = c_j * t_trend[1:].unsqueeze(1)
        mu_jm1 = c_jm1 * t_trend[:-1].unsqueeze(1)
        
        # 3. Calculate exact theoretical shift: mu_j - A * mu_{j-1}
    ...
    def pca_warm_start(self, subjects_data):
        with torch.no_grad():
            ...
            # Fallback: if an entire column is NaN, fill its mean with 0
            ...
            # 4. Scale SVD to get initial loadings
...
def simulate_ad_cohort_stress(N, D, K, C_dim, theta_mode="exact", seed=42, missing_rate=0.0, noise_scale=1.0, anchor_items=None):
    ...
    for _ in range(N):
        ...
        # Simulation strictly follows the discrete-time shift from the SDE
