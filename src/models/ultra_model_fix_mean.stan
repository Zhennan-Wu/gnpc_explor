...
parameters {
    ...
    // FREELY ESTIMATED LOADINGS (K-2 items, since 2 are anchored)
    ...
    // --- Structural Parameters (Corrected Scale/Corr split) ---
...
transformed parameters {
    matrix[Nsub, K] b;
    vector[K] lambda; // Full loadings vector
    ...
    // 1. Construct Full Loadings Vector (Anchoring the scale)
    // Factor 1
    lambda[1] = 1.0;              // ANCHOR
    ...
    Omega = 0.5 * (Omega + Omega'); // Symmetrize to fix floating point drift
    Omega = add_diag(Omega, 1e-5);  // Add tiny nugget for absolute positive-definite safety
    
    // 5. Generate Latent Trajectories (Joint Precision Equivalency)
    {
        ...
        for (i in 1:Nsub) {
            ...
            for (j in 2:repme[i]) {
                ...
                matrix[R, R] Q_sym = 0.5 * (Q + Q'); // Symmetrize
...
model {
    ...
    lambda_free ~ normal(1, sigma_lambda); // Apply prior to freely estimated parameters only
    ...
    L_S_scale ~ lognormal(0, 0.5); // Tight prior to prevent exploding drift during warmup
    ...
    to_vector(xi_raw) ~ std_normal(); // Standard normal prior for non-centered states
