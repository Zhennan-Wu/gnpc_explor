...
# ---------------------------------------------------------
# 4. Evaluation Wrappers (Smoke Test & Full Stress Test)
# ---------------------------------------------------------
def run_smoke_test():
    ...
    for s in scenarios:
        for mode in ["exact", "diagonal"]:
            ...
            for run_idx in range(n_runs):
                logging.info(f"Running {s['name']} | Mode: {mode.capitalize()} | Run: {run_idx+1}/{n_runs} ...")
                
                subjects_data, true_params = simulate_ad_cohort_stress(
                    s["N"], s["D"], s["K"], s["C"], 
                    theta_mode=mode, 
                    seed=400+run_idx,
                    missing_visit_rate=s["miss"], 
                    noise_scale=s["noise"]
                )
            ...
            result_row = f"{s['name']:<28} | {mode.capitalize():<10} | {l_str:<15} | {f_str:<15} | {g_str:<15} | {np.mean(run_times):>8.1f}"
