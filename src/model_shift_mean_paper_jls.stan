// Model 2a: Longitudinal IRT with SDE Latent Process & Time Trend (Joint Latent Sampler)
    ...
    // --- NON-CENTERED JOINT SAMPLER: Deterministic Construction ---
    {
        ...
        for (i in 1:Nsub) {
            ...
            for (j in 2:repme[i]) {
                ...
                vector[R] target_k = c_latent * time[k];
                vector[R] target_prev = c_latent * time[k-1];
