...
# ---------------------------------------------------------
# 1. GPU-Accelerated Universal DFOULS Model (Heterogeneous Noise + Multi-start)
# ---------------------------------------------------------
class Universal_DFOULS(nn.Module):
    ...
    def kalman_smoother(self, x_obs, A_trans, b_shift, dt, Lambda):
        ...
        # [OPTIMIZATION 1]: Treat R^-1 as a 1D vector to avoid a DxD diagonal matrix
        inv_psi = torch.exp(-self.log_psi)
        
        # Precompute terms that do not change over time j
        # Lambda^T * R^-1 (Broadcasting avoids DxD matrices, Shape: [K, D])
        ...
        for j in range(1, T):
            ...
            # Process noise dt[idx]*I_k ensures P_pred is positive definite
            ...
            if torch.isnan(x_obs[j]).all():
                ...
                # [OPTIMIZATION 2]: Woodbury Identity & Push-through
    ...
    def expected_complete_log_posterior_vectorized(self, subjects_data, smoothed_stats, Theta, Lambda):
        ...
        # [OPTIMIZATION 3]: Element-wise broadcasting avoids a DxD matrix creation
        ...
        for i, subj in enumerate(subjects_data):
            ...
            if valid_mask.any():
                ...
                trace_E = torch.sum(P_v * L_Psi_L, dim=(1,2)) + torch.sum(f_v * (f_v @ L_Psi_L), dim=1)
        ...
        return ll_obs + ll_lat + log_prior_Z + log_prior_dyn + log_prior_lin + log_prior_psi
...
# ---------------------------------------------------------
# 2. Disease Progression Data Simulation (UNCHANGED)
# ---------------------------------------------------------
...
def run_single_task(task_id, n_runs, out_dir):
    ...
    # Call the new multistart fit function
    ...
    result = {
        ...
        'parameters': {
            ...
            'est_params': {
                ...
                'Psi': torch.exp(model.log_psi).detach().cpu().numpy()  # Extract newly added log_psi
            },
...
def merge_results(out_dir):
    ...
    df_summary.to_csv("metrics_summary_het_multi.csv", index=False)
    torch.save(parameter_archive, "parameter_archive_het_multi.pt")
    
    print(f"[+] Merged {len(file_paths)} tasks.")
    print("[+] Saved 'metrics_summary_het_multi.csv' and 'parameter_archive_het_multi.pt'")
...
if __name__ == "__main__":
    ...
    if args.merge:
        ...
        # Run a specific job array task
        run_single_task(args.task_id, args.n_runs, args.out_dir)
    else:
        # Fallback: Run everything sequentially if no task_id provided (for local testing)
