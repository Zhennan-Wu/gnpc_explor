...
# ---------------------------------------------------------
LOG_FILENAME = "clouds_anchor_simulation_gem7.log"
logging.basicConfig(
    level=logging.INFO,
...
# ---------------------------------------------------------
# 1. Robust SPD Numerical Layer & Discovery Math
# ---------------------------------------------------------
def safe_cholesky(A, jitter=1e-5, max_jitter=1e-1):
    """Computes Cholesky with adaptive jitter for stiff covariance matrices."""
    try:
        return torch.linalg.cholesky(A)
    except RuntimeError:
        diag = torch.eye(A.shape[-1], device=A.device)
        current_jitter = jitter
        while current_jitter <= max_jitter:
            try:
                return torch.linalg.cholesky(A + current_jitter * diag)
            except RuntimeError:
                current_jitter *= 10
        return torch.linalg.cholesky(A + max_jitter * diag)

def adaptive_spd_inverse(A, threshold=100, num_iters=6):
    """Safely inverts SPD matrices using Cholesky or bounded Newton-Schulz."""
    is_unbatched = A.dim() == 2
    A_batched = A.unsqueeze(0) if is_unbatched else A
    B, N, _ = A_batched.shape
    I = torch.eye(N, device=A.device).unsqueeze(0).expand(B, N, N)
    
    if N <= threshold:
        L = safe_cholesky(A_batched)
        L_inv = torch.linalg.solve_triangular(L, I, upper=False)
        inv = L_inv.transpose(-2, -1) @ L_inv
    else:
        frob_norm_sq = torch.sum(A_batched * A_batched, dim=(-2, -1), keepdim=True)
        X = A_batched.transpose(-2, -1) / (frob_norm_sq + 1e-6)
        for _ in range(num_iters):
            AX = torch.bmm(A_batched, X)
            X = torch.bmm(X, (2.0 * I) - AX)
            
        res = torch.norm(torch.bmm(A_batched, X) - I, dim=(-2, -1)) / torch.norm(I, dim=(-2,-1))
        if torch.any(res > 1e-2) or torch.any(torch.isnan(X)):
            L = safe_cholesky(A_batched)
            L_inv = torch.linalg.solve_triangular(L, I, upper=False)
            X = L_inv.transpose(-2, -1) @ L_inv
        inv = X
        
    return inv.squeeze(0) if is_unbatched else inv
...
# ---------------------------------------------------------
# 2. CLOUDS Model (Statistically Coherent Architecture)
# ---------------------------------------------------------
class CLOUDS(nn.Module):
    ...
    def get_dynamics(self):
        ...
        if self.theta_mode == "exact":
            ...
            Omega_raw = L_omega @ L_omega.T + self.delta * torch.eye(self.K, device=device)
            stds = torch.sqrt(torch.diag(Omega_raw))
            Omega = Omega_raw / (stds.unsqueeze(1) * stds.unsqueeze(0))
            
            G = torch.tril(self.L_G)
            S = 0.5 * (G @ G.T) + self.delta * torch.eye(self.K, device=device)
            A_skew = self.gamma_skew - self.gamma_skew.T
            
            Omega_inv = adaptive_spd_inverse(Omega, threshold=100)
    ...
    def get_subject_matrices(self, Gamma, Omega, u_i, t):
        dt = t[1:] - t[:-1]
        if self.theta_mode == "diagonal":
            rho = torch.diag(Gamma)
            omega = torch.diag(Omega)
            a = torch.exp(-rho.unsqueeze(0) * dt.unsqueeze(1))
            A_trans = torch.diag_embed(a)
            Q = torch.diag_embed(omega.unsqueeze(0) * (1.0 - a**2))
        else:
        ...
        c_i = self.Phi_int @ u_i + self.alpha_bias
        mu_traj = c_i.unsqueeze(0) * t.unsqueeze(1)
        mu_j = mu_traj[1:]
        mu_jm1 = mu_traj[:-1]
        
        A_mu_jm1 = torch.bmm(A_trans, mu_jm1.unsqueeze(-1)).squeeze(-1)
        b_shift = mu_j - A_mu_jm1
        return A_trans, b_shift, dt, self.Lambda, Q, mu_traj

    def kalman_smoother(self, x_obs, A_trans, b_shift, dt, Lambda, Q, mu_traj, Omega):
        T = x_obs.shape[0]
        device = x_obs.device
        ...
        if not valid_mask_0.any():
            ...
            P_init_inv = adaptive_spd_inverse(P_init)
            
            P_filt[0] = adaptive_spd_inverse(P_init_inv + obs_info_0)
        ...
        for j in range(1, T):
            ...
            if not valid_mask_j.any():
                ...
                P_pred_inv = adaptive_spd_inverse(P_pred[j])
                
                P_filt[j] = adaptive_spd_inverse(P_pred_inv + obs_info_j)
        ...
        for j in range(T-2, -1, -1):
            P_pred_next_inv = adaptive_spd_inverse(P_pred[j+1])
            J_t = P_filt[j] @ A_trans[j].T @ P_pred_next_inv
    ...
    def evaluate_objective(self, subjects_data, smoothed_stats, Gamma, Omega, Lambda):
        ...
        inv_psi = torch.exp(-self.log_psi)
        Omega_stable = Omega + 1e-5 * torch.eye(self.K, device=Omega.device)
        Omega_inv = adaptive_spd_inverse(Omega_stable)
        log_det_Omega = torch.linalg.slogdet(Omega_stable)[1]
        
        for i, subj in enumerate(subjects_data):
            x_obs, u_i, t = subj['x'], subj['u'], subj['t']
            f_s, P_s, P_c = smoothed_stats[i]
            
            A_trans, b_shift, _, _, Q_exact, mu_traj = self.get_subject_matrices(Gamma, Omega, u_i, t)
            ...
            diff_0 = f_s[0] - mu_traj[0]
            M_0 = P_s[0] + torch.outer(diff_0, diff_0)
            ll_lat += -0.5 * log_det_Omega - 0.5 * torch.sum(Omega_inv * M_0)
            
            if len(t) > 1:
                Q_stable = Q_exact + 1e-5 * torch.eye(self.K, device=Q_exact.device).unsqueeze(0)
                Q_inv = adaptive_spd_inverse(Q_stable)
                ...
                M_j = (E_jj - torch.bmm(E_jjm1, A_T) - torch.bmm(A_trans, E_jjm1.transpose(1, 2)) + torch.bmm(A_trans, torch.bmm(E_jm1jm1, A_T)))
                ...
                M_j += (- torch.bmm(f_j_uns, b_T) - torch.bmm(b_uns, f_j_T) + torch.bmm(A_trans, torch.bmm(f_jm1_uns, b_T)) + torch.bmm(b_uns, torch.bmm(f_jm1_T, A_T)) + torch.bmm(b_uns, b_T))
    ...
    def calculate_bic(self, subjects_data, smoothed_stats):
        ...
        return -2 * log_likelihood.item() + p_total * np.log(N_total)
    ...
    def pca_warm_start(self, subjects_data):
        with torch.no_grad():
            ...
            self.Phi_int.data.fill_(0.0); self.alpha_bias.data.fill_(0.0)
            
            var_x = torch.mean(x_centered.square(), dim=0).clamp_min(1e-4)
            self.log_psi.data.copy_(torch.log(var_x))

    def fit_em_multistart(self, subjects_data, num_em_epochs=30, warmup_epochs=10, m_step_iters=10, lr=0.01, n_starts=3, burn_in_epochs=5):
        best_loss = float('inf')
        ...
        spatial_init_state = {p_name: p.clone() for p_name, p in self.named_parameters() if p_name in spatial_names}
        ...
        for start in range(n_starts):
            with torch.no_grad():
                ...
                for p_name, p in self.named_parameters():
                    if p_name in spatial_names:
                        p.data.copy_(spatial_init_state[p_name])
            ...
            for epoch in range(burn_in_epochs):
                ...
                with torch.no_grad():
                    for subj in subjects_data:
                        A_trans, b_shift, dt, _, Q, mu_traj = self.get_subject_matrices(Gamma, Omega, subj['u'], subj['t'])
                        smoothed_stats.append(self.kalman_smoother(subj['x'], A_trans, b_shift, dt, Lambda, Q, mu_traj, Omega))
        ...
        with torch.no_grad():
            for subj in subjects_data:
                A_trans, b_shift, dt, _, Q, mu_traj = self.get_subject_matrices(Gamma, Omega, subj['u'], subj['t'])
                final_smoothed_stats.append(self.kalman_smoother(subj['x'], A_trans, b_shift, dt, Lambda, Q, mu_traj, Omega))
                
        return final_smoothed_stats

# ---------------------------------------------------------
# 3. Data Simulation Wrapper (Equation-Consistent)
# ---------------------------------------------------------
def simulate_ad_cohort_stress(N, D, K, C_dim, theta_mode="exact", seed=42, missing_rate=0.0, noise_scale=1.0):
    ...
    subjects_data = []
    L_Omega_true = safe_cholesky(Omega_true + 1e-5*torch.eye(K))
    
    for _ in range(N):
        J_i = torch.randint(3, 6, (1,)).item()
        age_baseline = torch.rand(1) * 20 + 55
        dt_raw = torch.rand(J_i - 1) * 3.5 + 1.5
        times_raw = torch.cat([age_baseline, age_baseline + torch.cumsum(dt_raw, dim=0)])
        t_scaled = (times_raw - 70.0) / 10.0 
        
        u_i = torch.randn(C_dim)
        c_i = Phi_true @ u_i + alpha_true
        mu_traj = c_i.unsqueeze(0) * t_scaled.unsqueeze(1)
        
        F_true = torch.zeros(J_i, K)
        F_true[0] = mu_traj[0] + L_Omega_true @ torch.randn(K)
        
        for j in range(1, J_i):
            delta_t = t_scaled[j] - t_scaled[j-1]
            ...
            L_Q = safe_cholesky(Q_true)
            noise = L_Q @ torch.randn(K)
            
            F_true[j] = A_ij @ F_true[j-1] + mu_traj[j] - A_ij @ mu_traj[j-1] + noise
        ...
        subjects_data.append({'x': X_obs, 'u': u_i, 't': t_scaled, 'F_true': F_true})
    ...
    try:
        ...
        for k in scenario["test_Ks"]:
            ...
            sweep_results.append({
                "cand_k": k, "l_corr": l_corr, "f_corr": f_corr, "g_corr": g_corr, "bic": bic, "time": elapsed
            })
            
        return {"status": "success", "task_id": task_id, "scenario_name": scenario["name"], "true_k": true_K, "best_k": best_k, "sweep": sweep_results}
        
    except Exception as e:
        return {"status": "error", "task_id": task_id, "scenario_name": scenario["name"], "error_msg": str(e) + "\n" + traceback.format_exc()}

def run_comprehensive_pipeline_stress_test(n_runs=5):
    scenarios = [
        # 1. Low Dimensional Observation Space (D=500)
        {"name": "1a. Low D, Low K (D=500, K=4)",   "N": 150, "D": 500,  "K": 4,  "test_Ks": [3, 4, 5],    "C": 2, "miss": 0.1, "noise": 1.0},
        {"name": "1b. Low D, Mid K (D=500, K=8)",   "N": 200, "D": 500,  "K": 8,  "test_Ks": [7, 8, 9],    "C": 2, "miss": 0.1, "noise": 1.0},

        # 2. Standard Multi-Omics Space (D=2000)
        {"name": "2a. Std D, Low K (D=2000, K=4)",  "N": 150, "D": 2000, "K": 4,  "test_Ks": [3, 4, 5],    "C": 2, "miss": 0.1, "noise": 1.0},
        {"name": "2b. Std D, Mid K (D=2000, K=8)",  "N": 200, "D": 2000, "K": 8,  "test_Ks": [7, 8, 9],    "C": 2, "miss": 0.1, "noise": 1.0},
        {"name": "2c. Std D, High K (D=2000, K=12)","N": 250, "D": 2000, "K": 12, "test_Ks": [10, 12, 14], "C": 2, "miss": 0.1, "noise": 1.0},

        # 3. Massive Observation Space (D=5000)
        {"name": "3a. High D, Low K (D=5000, K=4)", "N": 150, "D": 5000, "K": 4,  "test_Ks": [3, 4, 5],    "C": 2, "miss": 0.1, "noise": 1.0},
        {"name": "3b. High D, High K (D=5000, K=12)","N": 250, "D": 5000, "K": 12, "test_Ks": [10, 12, 14], "C": 2, "miss": 0.1, "noise": 1.0},
    ]
    ...
    for s in scenarios:
        for run_idx in range(n_runs):
            tasks.append({"task_id": task_id, "scenario": s, "run_idx": run_idx, "seed": 2000 + run_idx + task_id})
            task_id += 1
    ...
    logging.info("=========================================================================================================")
    logging.info("  CLOUD FRAMEWORK: COMPREHENSIVE AUTOMATED DISCOVERY & SCALING STRESS TEST")
    logging.info("=========================================================================================================")
    
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_task = {executor.submit(run_single_discovery_sweep, t["task_id"], t["scenario"], t["run_idx"], t["seed"]): t for t in tasks}
    ...
    logging.info("\n" + "=" * 125)
    logging.info(f"{'Scenario':<34} | {'True K':<6} | {'Cand K':<6} | {'Select %':<8} | {'Λ Corr':<15} | {'F Corr':<15} | {'Γ Corr':<15} | {'Avg BIC':<12} | {'Avg Time'}")
    logging.info("-" * 125)
    ...
    for s in scenarios:
        for cand_k in s['test_Ks']:
            ...
            logging.info(f"{s['name']:<34} | {s['K']:<6} | {cand_k:<6} | {sel_pct:>3.0f}%    | {l_str:<15} | {f_str:<15} | {g_str:<15} | {avg_bic:<12.0f} | {avg_time:>8.1f}")
...
if __name__ == "__main__":
    ...
    # n_runs=5 is set for a balanced completion time on standard HPC blocks.
    run_comprehensive_pipeline_stress_test(n_runs=5)