...
Main fixes relative to the original version:
 ...
 22. Supports multiple anchor items per factor via anchor_groups.
 23. Can discover anchors automatically from rotated preliminary PCA loadings.
 24. Can select K by held-out visit predictive log likelihood with a one-SE rule.
 25. Adds combined K-selection safeguards: validation gap reporting, minimum anchor-stability gating, and explicit eligibility flags.
"""
...
# 0. Logging
# ---------------------------------------------------------------------
LOG_FILENAME = "clouds_anchor_simulation_chat7.log"
logging.basicConfig(
    level=logging.INFO,
...
class CLOUDS(nn.Module):
    def __init__(
        ...
        *,
        ...
        flat_anchor_rows, flat_anchor_factors, representative_rows = flatten_anchor_groups(self.anchor_groups)
        ...
        self.register_buffer("anchor_factor_idx", torch.tensor(flat_anchor_factors, dtype=torch.long))
        ...
        # For every row in anchor group r, only column r is active.
        ...
        # True at positive anchor entries: all anchors assigned to factor r are positive in column r.
    ...
    def _assign_lambda_from_matrix(self, Lambda_target: torch.Tensor) -> None:
        ...
        anchor_vals = Lambda_target[self.anchor_idx, self.anchor_factor_idx]
        self.Lambda_raw[self.anchor_idx, self.anchor_factor_idx] = torch.log(torch.abs(anchor_vals).clamp_min(1e-4))
    ...
    def pca_warm_start(self, subjects_data: Sequence[Dict[str, torch.Tensor]]) -> None:
        ...
        # Rotate PCA loadings toward the anchor coordinate system using one representative
        # anchor per factor. Additional anchors in each group are then enforced by the masks.
        ...
        self._assign_lambda_from_matrix(Lambda_rotated)
...
def simulate_ad_cohort_stress(
    ...
    *,
    ...
    anchor_groups_valid = normalize_anchor_groups(K, D, anchor_items=anchor_items, anchor_groups=anchor_groups)
    ...
    for r, group in enumerate(anchor_groups_valid):
        for idx in group:
            Lambda_true[idx, :] = 0.0
...
def preliminary_pca_loadings_from_matrix(x: torch.Tensor, K: int) -> Tuple[torch.Tensor, torch.Tensor]:
    """Return PCA loading estimate [D,K] and residual variances from a partially observed matrix."""
    if x.ndim != 2:
        raise ValueError("x must be a 2D matrix.")
    if x.shape[0] <= K:
        raise ValueError(f"Need more observed rows than K={K} for preliminary PCA.")
    x_filled, means = fill_nan_with_column_means(x)
    x_centered = x_filled - means.unsqueeze(0)
    U, S_vals, Vh = torch.linalg.svd(x_centered, full_matrices=False)
    n = x_centered.shape[0]
    K_eff = min(K, Vh.shape[0])
    loadings = torch.zeros(x_centered.shape[1], K, dtype=x.dtype, device=x.device)
    loadings[:, :K_eff] = Vh[:K_eff, :].T * torch.sqrt(S_vals[:K_eff].clamp_min(1e-12) / max(n - 1, 1))
    approx = (U[:, :K_eff] * S_vals[:K_eff]) @ Vh[:K_eff, :]
    resid = x_centered - approx
    psi = torch.mean(resid.square(), dim=0).clamp_min(1e-6)
    return loadings, psi


def preliminary_pca_loadings(subjects_data: Sequence[Dict[str, torch.Tensor]], K: int) -> Tuple[torch.Tensor, torch.Tensor]:
...
def varimax_rotation(
    ...
    *,
    ...
    """Orthogonal varimax rotation for simple-structure preliminary anchors."""
    D, K = loadings.shape
    if K <= 1:
    ...
    last_obj = torch.tensor(0.0, dtype=loadings.dtype, device=loadings.device)
    for _ in range(max_iter):
        L = loadings @ R
        col_norm = torch.sum(L.square(), dim=0, keepdim=True)
        target = L.pow(3) - (gamma / float(D)) * L * col_norm
        U, S_vals, Vh = torch.linalg.svd(loadings.T @ target, full_matrices=False)
        R = U @ Vh
        obj = torch.sum(S_vals)
...
def align_loadings_to_reference(loadings: torch.Tensor, reference: torch.Tensor) -> torch.Tensor:
    """Greedily permute/sign-flip columns of loadings to match reference columns."""
    D, K = reference.shape
    if loadings.shape != reference.shape:
        raise ValueError("loadings and reference must have the same shape.")
    L = loadings.clone()
    # Normalize columns for correlation-style matching.
    ref = reference - reference.mean(dim=0, keepdim=True)
    cand = L - L.mean(dim=0, keepdim=True)
    ref = ref / torch.sqrt(torch.sum(ref.square(), dim=0, keepdim=True)).clamp_min(1e-12)
    cand = cand / torch.sqrt(torch.sum(cand.square(), dim=0, keepdim=True)).clamp_min(1e-12)
    corr = cand.T @ ref  # [K candidate, K reference]

    assigned_cand = set()
    aligned = torch.zeros_like(reference)
    # Global greedy assignment by absolute correlation.
    pairs = []
    for c in range(K):
        for r in range(K):
            pairs.append((float(torch.abs(corr[c, r]).item()), c, r))
    pairs.sort(reverse=True)
    assigned_ref = set()
    for _, c, r in pairs:
        if c in assigned_cand or r in assigned_ref:
            continue
        sign = 1.0 if corr[c, r] >= 0 else -1.0
        aligned[:, r] = sign * L[:, c]
        assigned_cand.add(c)
        assigned_ref.add(r)
        if len(assigned_ref) == K:
            break
    # Fallback for pathological zero columns.
    remaining_c = [c for c in range(K) if c not in assigned_cand]
    remaining_r = [r for r in range(K) if r not in assigned_ref]
    for c, r in zip(remaining_c, remaining_r):
        aligned[:, r] = L[:, c]
    return aligned
...
def anchor_scores_from_loadings(
    ...
    *,
    ...
    """Return composite anchor scores plus purity, strength, communality."""
    absL = torch.abs(loadings)
    D, K = absL.shape
    total_sq = torch.sum(absL.square(), dim=1, keepdim=True)
    strength = absL
    cross_sq = (total_sq - absL.square()).clamp_min(0.0)
    cross_norm = torch.sqrt(cross_sq)
    purity = strength / (cross_norm + eps)
    if psi is None:
        communality = total_sq.squeeze(1) / (total_sq.squeeze(1) + 1.0)
    else:
        communality = total_sq.squeeze(1) / (total_sq.squeeze(1) + psi.to(loadings.device, loadings.dtype).clamp_min(eps))
...
def select_anchor_groups_from_loadings(
    ...
    *,
    ...
    """Select distinct anchor groups from a rotated loading matrix.

    The method first prefers items whose dominant loading belongs to the target
    factor and whose purity is above min_purity_ratio. If a factor does not have
    enough such items, it falls back to the highest-scoring unused rows. This
    makes the routine robust in noisy settings while still reporting low purity
    ...
    strength_cut = torch.quantile(strength.reshape(-1), float(min_strength_quantile)).item()

    groups: List[List[int]] = [[] for _ in range(K)]
    used = set()

    # First pass: strict simple-structure candidates.
    for r in range(K):
        order = torch.argsort(score[:, r], descending=True).cpu().tolist()
        for d in order:
            if len(groups[r]) >= anchors_per_factor:
                break
            if d in used:
                continue
            if int(dominant[d].item()) != r:
                continue
            if float(purity[d, r].item()) < min_purity_ratio:
                continue
            if float(strength[d, r].item()) < strength_cut:
                continue
            groups[r].append(int(d))
            used.add(int(d))

    # Second pass: relaxed non-overlap completion.
    for r in range(K):
        order = torch.argsort(score[:, r], descending=True).cpu().tolist()
        for d in order:
            if len(groups[r]) >= anchors_per_factor:
    ...
    if any(len(g) < anchors_per_factor for g in groups):
        raise RuntimeError("Unable to select enough distinct anchors. Reduce anchors_per_factor or K.")
...
def discover_anchor_groups(
    ...
    *,
    ...
    """Data-driven anchor discovery using rotated PCA loadings and optional bootstrap stability.

    Returns anchor_groups and diagnostics. This is a discovery stage only; the
    final CLOUDS fit still imposes hard anchor constraints.
    """
    x = stack_observed_rows(subjects_data)
    base_loadings, base_psi = preliminary_pca_loadings_from_matrix(x, K)
    if varimax:
    ...
    stability_matrix = torch.zeros(x.shape[1], K)
    chosen_stability: List[List[float]] = [[float("nan") for _ in g] for g in groups]
    if n_bootstrap > 0:
        rng = np.random.default_rng(seed)
        n_rows = x.shape[0]
        for _ in range(n_bootstrap):
            idx = torch.tensor(rng.integers(0, n_rows, size=n_rows), dtype=torch.long)
            boot_x = x[idx]
            try:
                boot_loadings, boot_psi = preliminary_pca_loadings_from_matrix(boot_x, K)
                if varimax:
                    boot_loadings, _ = varimax_rotation(boot_loadings)
                boot_loadings = align_loadings_to_reference(boot_loadings, base_loadings)
            ...
            for r, group in enumerate(boot_groups):
                for d in group:
                    stability_matrix[d, r] += 1.0
        stability_matrix = stability_matrix / float(max(n_bootstrap, 1))
        chosen_stability = []
        for r, group in enumerate(groups):
            chosen_stability.append([float(stability_matrix[d, r].item()) for d in group])
    ...
    score = info["score"]
    for r, group in enumerate(groups):
        group_purity.append([float(purity[d, r].item()) for d in group])
        group_strength.append([float(strength[d, r].item()) for d in group])
        group_scores.append([float(score[d, r].item()) for d in group])
    ...
    diagnostics: Dict[str, object] = {
        ...
        "mean_chosen_stability": float(np.nanmean([v for row in chosen_stability for v in row])) if n_bootstrap > 0 else float("nan"),
...
def lowrank_gaussian_loglik(
    ...
    *,
    ...
    """Log p(x_obs) for covariance diag(psi)+Lambda P Lambda^T using KxK Woodbury."""
    ...
    y = x[obs] - H @ mean
    logpsi = log_psi[obs]
    invpsi = torch.exp(-logpsi)
    Ht_invR = H.T * invpsi.unsqueeze(0)
    obs_info = Ht_invR @ H
    q = Ht_invR @ y
    eye_K = torch.eye(P.shape[0], device=x.device, dtype=x.dtype)
    P_stable = symmetrize(P) + jitter * eye_K
    P_inv = cholesky_spd_inverse(P_stable, jitter=jitter)
    M = symmetrize(P_inv + obs_info) + jitter * eye_K
    C = cholesky_spd_inverse(M, jitter=jitter)
    quad = torch.sum(y.square() * invpsi) - q @ C @ q
    logdet = torch.sum(logpsi) + torch.linalg.slogdet(eye_K + P_stable @ obs_info)[1]
...
def heldout_visit_predictive_loglik(
    ...
    """Evaluate held-out visit predictive log likelihood per observed cell.

    Held-out visits should be masked as NaN in train_subjects. The prediction for
    visit j is the Kalman one-step prediction before updating on that visit.
    """
    Gamma, Omega, _ = model.get_dynamics()
    Lambda = model.Lambda
    log_psi = model.log_psi
    record_map = {(int(r["subject_index"]), int(r["visit_index"])): r for r in holdout_records}
    total_ll = 0.0
    total_cells = 0
    device = Lambda.device
    dtype = Lambda.dtype
    eye_K = torch.eye(model.K, device=device, dtype=dtype)

    for i, subj in enumerate(train_subjects):
        keys_for_subject = [key for key in record_map if key[0] == i]
        if not keys_for_subject:
            continue
        t_dyn, t_trend = model._extract_times(subj)
        A_trans, b_shift, _, _, Q = model.get_subject_matrices(Gamma, Omega, subj["u"], t_dyn, t_trend)
        mu_path = model._mu_path(subj["u"], t_dyn)
        T = subj["x"].shape[0]
        inv_psi_full = torch.exp(-log_psi)
        full_Ht_invR = Lambda.T * inv_psi_full.unsqueeze(0)
        full_obs_info = full_Ht_invR @ Lambda
        f_pred = mu_path[0].to(device=device, dtype=dtype)
        P_pred = symmetrize(Omega.to(device=device, dtype=dtype)) + model.jitter * eye_K
        f_filt, P_filt = model._measurement_update(f_pred, P_pred, subj["x"][0], Lambda, full_obs_info, full_Ht_invR)
        if (i, 0) in record_map:
            rec = record_map[(i, 0)]
            ll, cells = lowrank_gaussian_loglik(rec["x_true"].to(device=device, dtype=dtype), f_pred, P_pred, Lambda, log_psi, jitter=model.jitter)
            total_ll += float(ll.item())
            total_cells += cells
        for j in range(1, T):
            f_pred = A_trans[j - 1] @ f_filt + b_shift[j - 1]
            P_pred = symmetrize(A_trans[j - 1] @ P_filt @ A_trans[j - 1].T + Q[j - 1])
            if (i, j) in record_map:
                rec = record_map[(i, j)]
                ll, cells = lowrank_gaussian_loglik(rec["x_true"].to(device=device, dtype=dtype), f_pred, P_pred, Lambda, log_psi, jitter=model.jitter)
                total_ll += float(ll.item())
                total_cells += cells
            f_filt, P_filt = model._measurement_update(f_pred, P_pred, subj["x"][j], Lambda, full_obs_info, full_Ht_invR)
    per_cell = total_ll / float(max(total_cells, 1))
    return total_ll, total_cells, per_cell
...
def choose_mode_for_k(
    ...
    *,
    ...
    """Gate dense exact Gamma unless temporal sample size is adequate."""
    ...
    enough_transitions = n_transitions >= int(math.ceil(min_trans_per_gamma_param * K * K))
    return "exact" if (K <= exact_max_k and enough_transitions) else "diagonal"


def count_transitions(subjects_data: Sequence[Dict[str, torch.Tensor]]) -> int:
    return int(sum(max(0, int(s["x"].shape[0]) - 1) for s in subjects_data))
...
def fit_clouds_candidate(
    ...
    *,
    ...
    """Fit a candidate K/mode model using the same continuation recipe as the stress tests."""
    recipe = adaptive_fit_recipe(K, scenario)
    reg = regularization_defaults_for_k(K, scenario)
    model_kwargs = dict(
        obs_dim=D,
...
def _finite_float(value: object, default: float = float("nan")) -> float:
    """Safely convert scalars to finite/non-finite floats for selection diagnostics."""
    try:
        out = float(value)
    except Exception:
        return default
...
def combined_k_selection_choice(
    ...
    *,
    ...
    """Choose K using validation one-SE plus optional anchor-stability safeguards.

    The primary rule is:

        smallest K such that
            mean validation ll/cell >= best mean - one_se_multiplier * SE(best),
            number of successful runs >= min_success_runs,
            and, if enabled, mean anchor stability >= min_anchor_stability.

    If no candidate passes the anchor-stability gate, the function falls back to
    the old one-SE rule and records the fallback reason. The rows are annotated
    in-place with diagnostic fields used by the final table.
    """
    successful = [r for r in results if r.get("status") == "success"]
    if not successful:
        raise RuntimeError("No successful K candidates to select from.")

    min_success_runs = max(1, int(min_success_runs))
    one_se_multiplier = max(0.0, float(one_se_multiplier))
    min_anchor_stability = float(min_anchor_stability)
    ...
    for r in results:
        if r.get("status") != "success":
            r["val_ll_gap_from_best"] = float("nan")
            r["within_one_se"] = False
            r["anchor_ok"] = False
            r["success_runs_ok"] = False
            r["combined_eligible"] = False
            continue

        mean_ll = float(r["val_ll_per_cell_mean"])
        stability = _finite_float(r.get("mean_anchor_stability", float("nan")))
        n_success = int(r.get("n_success", 0))
        within_one_se = mean_ll >= threshold
        success_runs_ok = n_success >= min_success_runs

        if not require_anchor_stability:
            anchor_ok = True
        else:
            anchor_ok = math.isfinite(stability) and stability >= min_anchor_stability

        r["val_ll_gap_from_best"] = best_mean - mean_ll
        r["within_one_se"] = bool(within_one_se)
        r["anchor_ok"] = bool(anchor_ok)
        r["success_runs_ok"] = bool(success_runs_ok)
        r["combined_eligible"] = bool(within_one_se and success_runs_ok and anchor_ok)

    eligible = [r for r in successful if bool(r.get("combined_eligible", False))]
    eligible.sort(key=lambda r: (int(r["K"]), 0 if str(r["mode"]) == "diagonal" else 1))

    if eligible:
        chosen = dict(eligible[0])
        selection_rule = "combined-one-SE-anchor" if require_anchor_stability else "one-SE"
        fallback_reason = ""
    else:
        one_se_eligible = [
            r for r in successful
            if bool(r.get("within_one_se", False)) and bool(r.get("success_runs_ok", False))
        ]
        one_se_eligible.sort(key=lambda r: (int(r["K"]), 0 if str(r["mode"]) == "diagonal" else 1))
        if one_se_eligible:
            chosen = dict(one_se_eligible[0])
            selection_rule = "fallback-one-SE"
            fallback_reason = (
                "No candidate passed the anchor-stability gate; selected by validation one-SE rule. "
                "Increase --anchor-bootstrap, lower --selection-min-anchor-stability, or inspect anchors manually."
            )
        else:
            # Last-resort fallback: choose the best mean validation candidate.
            chosen = dict(best)
            selection_rule = "fallback-best-mean"
            fallback_reason = (
                "No candidate passed the combined rule or the one-SE success-run rule; "
                "selected the best mean validation candidate."
            )

    chosen["one_se_threshold"] = threshold
    chosen["one_se_multiplier"] = one_se_multiplier
    chosen["best_K_by_mean"] = int(best["K"])
    chosen["best_mode_by_mean"] = str(best["mode"])
    chosen["best_val_ll_per_cell_mean"] = best_mean
    chosen["best_val_ll_per_cell_se"] = best_se
    chosen["min_anchor_stability"] = min_anchor_stability
    chosen["require_anchor_stability"] = bool(require_anchor_stability)
    chosen["min_success_runs"] = min_success_runs
    chosen["selection_rule"] = selection_rule
    chosen["fallback_reason"] = fallback_reason
    return chosen
...
def run_k_selection_experiment(
    *,
    ...
    """Synthetic unknown-K workflow: discover anchors, fit K grid, choose K by held-out visits."""
    logging.info("\n" + "=" * 160)
    logging.info("K-SELECTION EXPERIMENT WITH AUTOMATIC ANCHOR DISCOVERY")
    logging.info("=" * 160)
    logging.info(
        "True K=%s | candidate K=%s | true mode=%s | selection mode=%s | anchors/factor=%s",
        true_K,
        list(k_grid),
        true_mode,
        selection_mode,
    ...
    logging.info(
        "Selection safeguards: one-SE multiplier=%.2f | min anchor stability=%.3f | require anchor stability=%s | min successful runs=%s",
        float(one_se_multiplier),
        float(min_anchor_stability),
        bool(require_anchor_stability),
        int(min_success_runs),
    )

    # Synthetic truth uses multiple anchors per true factor so anchor discovery is well-defined.
    true_anchor_groups = []
    next_anchor = 0
    for _ in range(true_K):
        group = list(range(next_anchor, min(next_anchor + anchors_per_factor, D)))
        if len(group) < anchors_per_factor:
            raise ValueError("D is too small for true_K * anchors_per_factor synthetic anchors.")
        true_anchor_groups.append(group)
        next_anchor += anchors_per_factor

    # Results are stored per candidate K across independent simulated cohorts.
    ...
    for run_idx in range(n_runs):
        ...
        subjects, _ = simulate_ad_cohort_stress(
            ...
            K=true_K,
            C_dim=C,
            theta_mode=true_mode,
            seed=seed,
            noise_scale=1.0,
            anchor_groups=true_anchor_groups,
            exact_target_rate=1.0,
            exact_skew_strength=0.05,
            visit_min=5,
            visit_max=8,
        )
        train_subjects, holdouts = make_visit_holdout_split(
            subjects,
            validation_fraction=validation_fraction,
            seed=seed + 17,
            holdout="last",
        )
        n_trans = count_transitions(train_subjects)
        logging.info("Run %s/%s: held out %s visits; transitions=%s", run_idx + 1, n_runs, len(holdouts), n_trans)

        for K in k_grid:
            K = int(K)
            candidate_start = time.time()
            try:
                mode = choose_mode_for_k(
                    K,
                    n_trans,
                ...
                anchor_groups, anchor_info = discover_anchor_groups(
                    train_subjects,
                    K,
                    anchors_per_factor=anchors_per_factor,
                    n_bootstrap=anchor_bootstrap,
                    seed=seed + 1000 + K,
                ...
                _, n_cells, val_per_cell = heldout_visit_predictive_loglik(model, train_subjects, holdouts)
                elapsed = time.time() - candidate_start
                rec = {
                    "status": "success",
                    "K": K,
                    "mode": mode,
                    "val_ll_per_cell": val_per_cell,
                    "n_cells": n_cells,
                    "time": elapsed,
                    "mean_anchor_stability": anchor_info.get("mean_chosen_stability", float("nan")),
                    "anchor_groups": anchor_groups,
                }
                per_candidate.setdefault((K, mode), []).append(rec)
                logging.info(
                    "  K=%s | %s | val ll/cell=%.4f | anchor stability=%.3f | time=%.1fs | anchors=%s",
                    K,
                    mode,
                    val_per_cell,
                    float(rec["mean_anchor_stability"]),
                    elapsed,
                    anchor_groups,
                )
                del model
                ...
                per_candidate.setdefault((K, mode), []).append(rec)
                logging.error("  K=%s | %s failed: %s", K, mode, traceback.format_exc())
            gc.collect()

    # Aggregate candidate results.
    rows: List[Dict[str, object]] = []
    for (K, mode), runs in sorted(per_candidate.items(), key=lambda kv: (kv[0][0], kv[0][1])):
        ok = [r for r in runs if r.get("status") == "success"]
        if not ok:
            rows.append({"status": "error", "K": K, "mode": mode, "n_success": 0})
            continue
        vals = np.array([float(r["val_ll_per_cell"]) for r in ok], dtype=float)
        times = np.array([float(r["time"]) for r in ok], dtype=float)
        stabs = np.array([float(r["mean_anchor_stability"]) for r in ok], dtype=float)
        rows.append({
            "status": "success",
            "K": K,
            "mode": mode,
            ...
            "example_anchors": ok[0].get("anchor_groups"),
        })
    ...
    logging.info("\n" + "=" * 160)
    logging.info("FINAL K-SELECTION RESULTS")
    logging.info("=" * 160)
    header = (
        f"{'K':>4} | {'Mode':<9} | {'Runs':>4} | {'Val ll/cell mean +/- SE':<28} | "
        f"{'Gap best':>9} | {'1SE':>3} | {'Anchor':>8} | {'AOK':>3} | {'Elig':>5} | "
        f"{'Avg time(s)':>11} | Example anchor groups"
    )
    logging.info(header)
    logging.info("-" * 180)
    for r in rows:
        if r.get("status") != "success":
            logging.info(f"{int(r['K']):>4} | {str(r['mode']).capitalize():<9} | {'0':>4} | FAILED")
            continue
        val_str = f"{float(r['val_ll_per_cell_mean']):.4f} +/- {float(r['val_ll_per_cell_se']):.4f}"
        gap = _finite_float(r.get("val_ll_gap_from_best", float("nan")))
        gap_str = "nan" if not math.isfinite(gap) else f"{gap:.4f}"
        stab = _finite_float(r.get("mean_anchor_stability", float("nan")))
        stab_str = "nan" if not math.isfinite(stab) else f"{stab:.3f}"
        one_se_flag = "Y" if bool(r.get("within_one_se", False)) else "N"
        anchor_flag = "Y" if bool(r.get("anchor_ok", False)) else "N"
        eligible_flag = "Y" if bool(r.get("combined_eligible", False)) else "N"
        logging.info(
            f"{int(r['K']):>4} | {str(r['mode']).capitalize():<9} | {int(r['n_success']):>4} | "
            f"{val_str:<28} | {gap_str:>9} | {one_se_flag:>3} | {stab_str:>8} | "
            f"{anchor_flag:>3} | {eligible_flag:>5} | {float(r['mean_time']):>11.1f} | {r.get('example_anchors')}"
        )
    logging.info("-" * 180)
    logging.info(
        "Selected by %s: K=%s, mode=%s. Best mean candidate was K=%s, mode=%s. Threshold=%.4f; best mean=%.4f +/- %.4f; min anchor stability=%.3f",
        str(chosen["selection_rule"]),
        int(chosen["K"]),
        str(chosen["mode"]),
        int(chosen["best_K_by_mean"]),
        str(chosen["best_mode_by_mean"]),
        float(chosen["one_se_threshold"]),
        float(chosen["best_val_ll_per_cell_mean"]),
        float(chosen["best_val_ll_per_cell_se"]),
        float(chosen["min_anchor_stability"]),
    )
    if str(chosen.get("fallback_reason", "")):
        logging.warning(str(chosen["fallback_reason"]))
    logging.info("=" * 160)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run CLOUDS stress simulations, latent scaling, and unknown-K selection.")
    parser.add_argument(
        "--experiment",
        choices=["baseline", "latent", "select", "both"],
        default=os.environ.get("CLOUDS_EXPERIMENT", "latent"),
        help="Which experiment suite to run. Use select for automatic K/anchor selection. Default: env CLOUDS_EXPERIMENT or latent.",
    )
    parser.add_argument(
        "--n-runs",
        type=int,
    ...
    parser.add_argument(
        "--selection-k-grid",
        default=os.environ.get("CLOUDS_SELECTION_K_GRID", "2,3,4,6,8,12,16"),
        help="Comma-separated candidate K values for --experiment select. Default: 2,3,4,6,8,12,16.",
    )
    ...
    parser.add_argument(
        "--selection-true-mode",
        choices=["exact", "diagonal"],
        default=os.environ.get("CLOUDS_SELECTION_TRUE_MODE", "exact"),
        help="Synthetic DGP mode for K-selection validation. Default: exact.",
    )
    parser.add_argument(
        "--selection-mode",
        choices=["auto", "exact", "diagonal"],
        default=os.environ.get("CLOUDS_SELECTION_MODE", "auto"),
        help="Candidate fitting mode. auto gates dense exact Gamma by K and transition count. Default: auto.",
    )
    parser.add_argument(
        "--anchors-per-factor",
        type=int,
        default=int(os.environ.get("CLOUDS_ANCHORS_PER_FACTOR", "3")),
        help="Number of automatically discovered hard anchors per factor. Default: 3.",
    )
    parser.add_argument(
        "--anchor-bootstrap",
        type=int,
        default=int(os.environ.get("CLOUDS_ANCHOR_BOOTSTRAP", "3")),
        help="Bootstrap replicates for anchor stability diagnostics. Use 0 to disable. Default: 3.",
    )
    parser.add_argument(
        "--validation-fraction",
        type=float,
        default=float(os.environ.get("CLOUDS_VALIDATION_FRACTION", "0.25")),
        help="Fraction of subjects with one held-out validation visit. Default: 0.25.",
    )
    parser.add_argument(
        "--selection-profile",
        choices=["fast", "standard", "thorough"],
        default=os.environ.get("CLOUDS_SELECTION_PROFILE", "fast"),
        help="Fit budget for K-selection candidates. Default: fast.",
    )
    parser.add_argument(
        "--selection-exact-max-k",
        type=int,
        default=int(os.environ.get("CLOUDS_SELECTION_EXACT_MAX_K", "8")),
        help="In auto mode, allow dense exact Gamma only up to this K if transitions are sufficient. Default: 8.",
    )
    parser.add_argument(
        "--selection-min-trans-per-gamma-param",
        type=float,
        default=float(os.environ.get("CLOUDS_SELECTION_MIN_TRANS_PER_GAMMA_PARAM", "5.0")),
        help="Auto-mode transition-count gate: require transitions >= this * K^2 for dense exact Gamma. Default: 5.0.",
    )
    parser.add_argument(
        "--selection-min-anchor-stability",
        type=float,
        default=float(os.environ.get("CLOUDS_SELECTION_MIN_ANCHOR_STABILITY", "0.5")),
        help="Minimum mean bootstrap anchor stability required for K-selection eligibility. Default: 0.5.",
    )
    parser.add_argument(
        "--selection-disable-anchor-stability-gate",
        action="store_true",
        help="Disable the anchor-stability eligibility gate and select by validation one-SE only.",
    )
    parser.add_argument(
        "--selection-one-se-multiplier",
        type=float,
        default=float(os.environ.get("CLOUDS_SELECTION_ONE_SE_MULTIPLIER", "1.0")),
        help="Multiplier applied to the best candidate SE in the one-SE rule. Default: 1.0.",
    )
    parser.add_argument(
        "--selection-min-success-runs",
        type=int,
        default=int(os.environ.get("CLOUDS_SELECTION_MIN_SUCCESS_RUNS", "1")),
        help="Minimum successful replicate fits required for a K candidate to be eligible. Default: 1.",
    )
    ...
    if args.experiment in {"select", "both"}:
        run_k_selection_experiment(
            true_K=args.selection_true_k,
            k_grid=parse_k_grid(args.selection_k_grid),
            n_runs=args.n_runs,
            N=args.selection_n,
            D=args.selection_d,
            C=args.selection_covar_dim,
            true_mode=args.selection_true_mode,
